...
- Stories from the old site were ported by `scripts/port_legacy_pages.py`
  from `migration/pages.yml` (legacy path, new path, date, byline, or
  `action: skip` with the reason). News pages carry a `date:` key, and the
  Archive at the bottom of `docs/news/index.md` (between `archive:start` /
  `archive:end` markers) is regenerated from it by that script.
