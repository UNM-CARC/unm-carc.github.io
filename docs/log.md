...
* **News archive**: 45 stories from the old website (2016–2024) — conference trips from SC17 to SC23, the annual user meetings, staff and student arrivals, the Hopper launch, ColdFront adoption, the $1.5M workforce-development award, the Vis Lab and gallery stories, and the COVID-19 research pieces — now live under [News](news/index.md), grouped by year in a generated Archive, with their photographs localized. They were converted by a new one-shot tool, `scripts/port_legacy_pages.py`, from `migration/pages.yml`, which also records the eight old stories deliberately left behind (expired event notices, a duplicate, and undated orphans). Each ported page records its old address, so the old URLs redirect. Links to the ported stories from [Featured projects](research/featured-projects.md) and [History](about/history.md) now stay on this site.
...
* **Navigation**: Compacted the centered user-actions row so its buttons remain on one line at desktop widths while continuing to wrap on narrow screens.
* **Navigation**: The header now separates CARC sections from a centered user-actions row: [Site Feedback](https://github.com/UNM-CARC/unm-carc.github.io/issues){target=_blank}, [User Documentation](/docs/), [Open OnDemand](https://ood.alliance.unm.edu/){target=_blank}, a Systems Status menu with [HPC Status](https://vortex.alliance.unm.edu/public-dashboards/d1573fdbc5164b2dbf3cdfdcfe437f63){target=_blank}, [Service Uptime](https://stats.uptimerobot.com/kqt0LYLwFd){target=_blank}, and [Downtime Notices](https://carc.unm.edu/systems/downtime-notices.html), plus [ColdFront](https://coldfront.alliance.unm.edu/){target=_blank}, [Help Desk](https://support.alliance.unm.edu/){target=_blank}, and [Office Hours](https://carc.unm.edu/user-support-2/office-and-consultation-hours.html).
...
* **Initialization**: Created this rebuild of [carc.unm.edu](https://carc.unm.edu) with [Zensical](https://zensical.org), structured as an Open Knowledge Format (OKF v0.2) bundle with the same agent surface as the [user documentation](/docs/): llms.txt, per-page Markdown mirrors, and an origin-wide robots.txt.
